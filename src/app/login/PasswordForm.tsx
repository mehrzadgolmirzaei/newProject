...
export function PasswordForm({ next, allowRegister }: { next: string; allowRegister: boolean }) {
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if (mode === "register" && password !== repeat) return setError({ text: t("تکرار رمز عبور با رمز عبور یکسان نیست."), field: "repeat" });
    start(async () => {
  ...
  return (
    <form onSubmit={submit} className="stack gap-16 auth-form">
      <div>
        <h1>{mode === "login" ? t("ورود پزشکان") : t("ثبت‌نام پزشکان")}</h1>
        <p className="sub">
          {mode === "login"
            ? t("برای ثبت تشخیص، شرکت در گفت‌وگوی علمی و پیگیری پیشرفت خود وارد شوید.")
            : t("حساب کاربری ایجاد کنید. پس از تکمیل مشخصات پزشکی، حساب شما توسط مدیر سامانه تأیید می‌شود.")}
        </p>
      </div>

      {allowRegister && (
        <div className="seg" role="tablist" aria-label={t("نوع ورود")}>
          <button type="button" role="tab" aria-selected={mode === "login"} onClick={() => switchTo("login")}>{t("ورود")}</button>
          <button type="button" role="tab" aria-selected={mode === "register"} onClick={() => switchTo("register")}>{t("ثبت‌نام")}</button>
        </div>
      )}

      <div className="field">
        <label htmlFor="username">{t("نام کاربری")}</label>
        <input
        ...
        />
        {mode === "register" && <span className="hint">{t("حروف لاتین کوچک، عدد، نقطه یا زیرخط؛ با حرف شروع شود.")}</span>}
      </div>
      ...
      <div className="field">
        ...
        <div className="input-group">
          ...
          />
          <button type="button" className="input-addon" onClick={() => setShow((s) => !s)} aria-label={show ? t("پنهان‌کردن رمز") : t("نمایش رمز")}>
            <Icon name="eye" />
          </button>
        </div>
        {mode === "register" && <span className="hint">{t("دست‌کم ۸ نویسه.")}</span>}
      </div>

      {mode === "register" && (
        <div className="field">
          <label htmlFor="repeat">{t("تکرار رمز عبور")}</label>
          <input
      ...
      <button className="btn btn-primary btn-lg btn-block" disabled={pending || !username || !password}>
        {pending ? t("لطفاً صبر کنید…") : mode === "login" ? t("ورود") : t("ایجاد حساب")}
      </button>

      {mode === "login" && (
        <p className="muted" style={{ fontSize: 12.5, lineHeight: 1.9 }}>
          {t("در صورت فراموشی رمز عبور، با مدیر سامانه تماس بگیرید تا رمز جدیدی برای شما تعیین شود.")}
        </p>
      )}
    </form>
  );
