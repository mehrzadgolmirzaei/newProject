  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        setMsg(null);
        if (f.next !== f.repeat) return setMsg({ ok: false, text: t("تکرار رمز عبور با رمز جدید یکسان نیست.") });
        start(async () => {
          const r = await changePassword({ current: f.current, next: f.next });
          if (!r.ok) return setMsg({ ok: false, text: r.error });
          setF({ current: "", next: "", repeat: "" });
          setMsg({ ok: true, text: t("رمز عبور با موفقیت تغییر کرد.") });
        });
      }}
    >
      <h2 style={{ fontSize: 17, margin: 0 }}>{t("تغییر رمز عبور")}</h2>
      <input type="text" name="username" autoComplete="username" hidden readOnly />
      <div className="grid-3">
        <div className="field">
          <label htmlFor="pc">{t("رمز فعلی")}</label>
        ...
        <div className="field">
          <label htmlFor="pn">{t("رمز جدید")}</label>
        ...
        <div className="field">
          <label htmlFor="pr">{t("تکرار رمز جدید")}</label>
      ...
      <div><button className="btn btn-secondary" disabled={pending || !f.current || !f.next}>{pending ? t("در حال ذخیره…") : t("ذخیره‌ی رمز جدید")}</button></div>
    </form>
  );
}
