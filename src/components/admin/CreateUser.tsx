...
export function CreateUser() {
  ...
  if (!open)
    return (
      <div style={{ marginBottom: 16 }}>
        <button className="btn btn-secondary btn-sm" onClick={() => setOpen(true)}><Icon name="plus" /> ایجاد حساب جدید</button>
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        start(async () => {
          ...
          setMsg({ ok: true, text: `حساب ${f.username} ایجاد شد. نام کاربری و رمز عبور را به پزشک اطلاع دهید.` });
    ...
    >
      ...
      <p className="muted" style={{ fontSize: 12.5, margin: 0 }}>حساب بلافاصله فعال می‌شود. پزشک در نخستین ورود، مشخصات پزشکی خود را تکمیل می‌کند و می‌تواند رمز عبور را تغییر دهد.</p>
      {msg && !msg.ok && <div className="alert alert-danger"><Icon name="alert" />{msg.text}</div>}
      <div className="row gap-8">
        <button className="btn btn-primary btn-sm" disabled={pending || !f.username || !f.password}>{pending ? "در حال ایجاد…" : "ایجاد حساب"}</button>
        <button type="button" className="btn btn-ghost btn-sm" onClick={() => { setOpen(false); setMsg(null); }}>انصراف</button>
      </div>
    </form>
  );
}
